...
If the server requires authentication, OpenCode will prompt you to authenticate when you first try to use it. If OpenCode does not prompt you to authenticate, you can [manually authenticate](#authenticating) with `opencode mcp auth <server-name>` to trigger the OAuth flow.
...
If you're having trouble connecting to an OAuth-enabled MCP server, you can use these commands to diagnose issues:
...
The `mcp debug` command will:

- Show the current auth status and stored token info
- Test HTTP connectivity to the server
- Display the `WWW-Authenticate` header if present
- Attempt the OAuth discovery flow to verify it's working
