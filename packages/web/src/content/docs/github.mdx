...
The GitHub integration supports the following events:

- Mentions on issues and pull requests
- Pull request review comments (inline comments on files or lines)

Other event types, such as pull request reviews (approve/comment/changes) or when creating an issue, are not yet supported.
